...
bool TCPConn::IsConnected() const {
  ...
  if (!FD_ISSET(sock_, &rfd)) return false;
  int n = 0;
...
void TCPConn::WriteChar(int8_t c) { WriteBuf(&c, sizeof(c)); }

void TCPConn::WriteUChar(uint8_t c) { WriteBuf(&c, sizeof(c)); }

void TCPConn::WriteShort(int16_t s) { WriteBuf(&s, sizeof(s)); }

void TCPConn::WriteUShort(uint16_t s) { WriteBuf(&s, sizeof(s)); }

void TCPConn::WriteInt(int32_t i) { WriteBuf(&i, sizeof(i)); }

void TCPConn::WriteUInt(uint32_t i) { WriteBuf(&i, sizeof(i)); }

void TCPConn::WriteLong(int64_t l) { WriteBuf(&l, sizeof(l)); }

void TCPConn::WriteULong(uint64_t l) { WriteBuf(&l, sizeof(l)); }
